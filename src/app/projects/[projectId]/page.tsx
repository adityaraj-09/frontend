    ...
    <div className="h-full min-h-0 overflow-y-auto bg-background">
      <div className="flex w-full flex-col px-6 pt-5 pb-12">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            type="button"
            className="inline-flex h-8 items-center gap-1.5 rounded-full border border-border bg-background px-3 text-[13px] font-semibold"
        ...
        <div className="mt-6 flex items-center justify-between gap-3">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger aria-label="Project menu" className="flex size-8 items-center justify-center rounded-full hover:bg-muted">
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_280px]">
          ...
          <aside className="flex flex-col gap-3">
            <RailCard
              title="Memory"
              action={
                <button type="button" className="inline-flex items-center gap-1 text-[12px] font-semibold text-muted-foreground" onClick={() => void toggleMemory()}>
                  <Lock className="size-3" />
                  {row?.memoryEnabled === false ? "Off" : "Only you"}
                </button>
              }
            >
              {row?.memoryEnabled === false
                ? "Memory is off"
                : `${row?.memoryUsedPercent ?? 0}% of memory used`}
            </RailCard>
            <RailCard
              title="Instructions"
              action={<Settings className="size-3.5 text-muted-foreground" />}
            >
              <textarea
                value={text}
                ...
                className="mt-2 min-h-20 w-full resize-none bg-transparent text-[13px] font-medium outline-none placeholder:text-muted-foreground"
              />
            </RailCard>
            <RailCard title="Files" action={<Plus className="size-3.5 text-muted-foreground" />}>
...
function RailCard({
  ...
  return (
    <section className="rounded-[16px] border border-border px-4 py-3">
      <div className="flex items-center justify-between text-[13px] font-semibold">
        {title}
        {action}
      </div>
      <div className="mt-1 text-[12px] font-medium text-muted-foreground">{children}</div>
    </section>
  );
}
