    ...
    >
      <Link
        ...
        className={cn(
          ...
          compact
            ? cn("px-2 py-1.5 text-[14px] font-normal leading-5", active ? "text-[#1b1b1b]" : "text-[#585858]")
            : "py-1 text-[14px] hover:text-black",
        )}
      >
        {chat.title || "New chat"}
        {!compact ? (
          <div className="text-[12px] text-[#a1a1aa]">{new Date(chat.lastMessageAt).toLocaleString()}</div>
        ) : null}
      </Link>
      <button
        type="button"
        ...
        aria-pressed={pinned}
        className={cn(
          "flex size-7 shrink-0 items-center justify-center rounded-md text-[#a1a1aa] hover:bg-black/[0.06] hover:text-[#1b1b1b]",
